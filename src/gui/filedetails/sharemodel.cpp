...
QVariant ShareModel::data(const QModelIndex &index, const int role) const
{
    ...
    qCWarning(lcShareModel) << "Got unknown role" << role <<  "returning null value.";
    return {};
}
...
void ShareModel::updateData()
{
    ...
    _sharePath = _localPath.mid(_folder->cleanPath().length() + 1);

    SyncJournalFileRecord fileRecord;
    bool resharingAllowed = true; // lets assume the good

    if (_folder->journalDb()->getFileRecord(_sharePath, &fileRecord) &&
        fileRecord.isValid() &&
